...
def find_closest_empty_tile(pos, game_state):
    ...
    for adjacent_position in adjacent_positions:
        try:
            if is_cell_empty(adjacent_position,game_state):
                return game_state.map.get_cell_by_pos(adjacent_position)
        except Exception:
            continue
...
def agent(observation, configuration):
    ...
    for unit in player.units:
        ...
        if unit.is_worker() and unit.can_act():
            closest_empty_tile = find_closest_empty_tile(unit.pos, game_state)
            ...
            elif steps_until_night > 10 and can_build and unit.get_cargo_space_left() <= 20 and is_position_resource(resource_tiles, unit.pos) and closest_empty_tile is not None:
