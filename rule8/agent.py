...
def find_closest_empty_tile(pos, game_state,player):
    adjacent_positions = [Position(pos.x,pos.y+1),Position(pos.x-1,pos.y),Position(pos.x,pos.y-1),Position(pos.x+1,pos.y)];
    empty_tyles=[]
...
def agent(observation, configuration):
    ...
    for unit in player.units:
        print("Unit",unit.id,";pos",unit.pos,'CD=',unit.cooldown,cargo_to_string(unit.cargo),'fuel=',cargo_to_fuel(unit.cargo),'canBuildHere',unit.can_build(game_state.map),file=sys.stderr)
