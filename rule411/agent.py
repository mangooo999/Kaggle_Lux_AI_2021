...
def agent(observation, configuration):
    ...
    for cluster in clusters.get_clusters():
        ...
        if cluster.res_type == RESOURCE_TYPES.WOOD and cluster.get_equivalent_resources() > 10 \
            ...
            for pos in cluster.perimeter_empty:
                ...
                unit_around = get_units_and_city_number_around_pos(player,pos,2)
                if unit_around > 0:
                    continue
                enemy_around = get_units_and_city_number_around_pos(opponent, pos, 2)
                if enemy_around > 0:
                    continue
